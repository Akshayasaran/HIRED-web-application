        ...
        if(cookies != null){
            for(Cookie cookie : cookies){
                if(cookie.getName().equals("message"))
                    if (cookie.getValue().equals("login_fail"))
                        out.println("<div class='alert alert-danger alert-dismissible fade show' role='alert'><strong>Invalid Email address or Password!</strong><button class='btn-close' type='button' data-bs-dismiss='alert' aria-label='Close'></button></div>");
                    if (cookie.getValue().equals("login_error"))
                    ...
                    if (cookie.getValue().equals("login_norecord"))
                        out.println("<div class='alert alert-primary alert-dismissible fade show' role='alert'><strong>User doesn't exist. Please register before login.</strong><button class='btn-close' type='button' data-bs-dismiss='alert' aria-label='Close'></button></div>");                
                    if (cookie.getValue().equals("login_first"))
                        out.println("<div class='alert alert-primary alert-dismissible fade show' role='alert'><strong>You should Login first.</strong><button class='btn-close' type='button' data-bs-dismiss='alert' aria-label='Close'></button></div>");                                    
                    if (cookie.getValue().equals("register_updated"))
                        out.println("<div class='alert alert-success alert-dismissible fade show' role='alert'><strong>User successfully Created.</strong><button class='btn-close' type='button' data-bs-dismiss='alert' aria-label='Close'></button></div>");                                    
              }
        }
      %>
            ...
            <div class="card">
              <div class="card-body p-4 p-sm-5">
                <div class="row flex-between-center mb-2">
                  ...
                  <div class="col-auto fs--1 text-600"><span class="mb-0 undefined">or</span> <span><a href="register.jsp ">Create an account</a></span></div>
                </div>
                  <form class="needs-validation" novalidate="" action="<%= request.getContextPath() %>/Login" method="POST">
                  <div class="mb-3"><input class="form-control" type="email" name="login_emailid" placeholder="Email address" required="" />
                    <div class="valid-feedback">Looks good!</div>
                    <div class="invalid-feedback">Enter a valid Email address</div>
                  </div>
                  <div class="mb-3"><input class="form-control" type="password" name="login_password" placeholder="Password" required />
                    <div class="valid-feedback">Looks good!</div>
                    <div class="invalid-feedback">Enter a valid Password</div>                  
                  </div>
                  <div class="btn-group" role="group" aria-label="Basic radio toggle button group">
                    <input type="radio" class="btn-check" name="login_usertype" id="btnradio1" autocomplete="off" checked value="Individual">
                    <label class="btn btn-outline-primary" for="btnradio1">Individual</label>
                  
                    <input type="radio" class="btn-check" name="login_usertype" id="btnradio2" autocomplete="off" value="Organisation">
                    <label class="btn btn-outline-primary" for="btnradio2">Organisation</label>
                  </div>
                  <div class="row flex-between-center">
                    <div class="col-auto">
                      <div class="form-check mb-0"><input class="form-check-input" type="checkbox" id="basic-checkbox" checked="checked" /><label class="form-check-label" for="basic-checkbox">Remember me</label></div>
                    </div>
...
<!-- Mirrored from prium.github.io/falcon/v3.0.0-beta4/authentication/basic/login.html by HTTrack Website Copier/3.x [XR&CO'2014], Fri, 19 Mar 2021 07:25:11 GMT -->
</html>